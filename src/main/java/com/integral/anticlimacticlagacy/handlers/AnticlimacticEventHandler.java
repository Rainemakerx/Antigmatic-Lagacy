...
import com.integral.anticlimacticlagacy.gui.EnderChestInventoryButton;
import com.integral.anticlimacticlagacy.gui.ToggleMagnetEffectsButton;
import com.integral.anticlimacticlagacy.helpers.BlueSkiesHelper;
import com.integral.anticlimacticlagacy.helpers.CrossbowHelper;
import com.integral.anticlimacticlagacy.helpers.AnticlimacticEnchantmentHelper;
...
import com.integral.anticlimacticlagacy.helpers.OverlayPositionHelper;
import com.integral.anticlimacticlagacy.helpers.OverlayPositionHelper.AnchorPoint;
import com.integral.anticlimacticlagacy.helpers.OverlayPositionHelper.OverlayPosition;
...
import com.integral.anticlimacticlagacy.mixin.AccessorAbstractArrowEntity;
import com.integral.anticlimacticlagacy.mixin.AccessorAdvancementCommands;
import com.integral.anticlimacticlagacy.objects.CooldownMap;
import com.integral.anticlimacticlagacy.objects.DamageSourceNemesisCurse;
...
import com.integral.anticlimacticlagacy.objects.QuarkHelper;
...
import com.integral.anticlimacticlagacy.packets.clients.PacketCosmicRevive;
...
import com.integral.anticlimacticlagacy.packets.server.PacketEnchantingGUI;
...
import static com.integral.anticlimacticlagacy.AnticlimacticLagacy.*;
import static com.integral.anticlimacticlagacy.objects.RegisteredMeleeAttack.getRegisteredMeleeAttack;
import static com.integral.anticlimacticlagacy.config.JsonConfigHandler.*;
...
public class AnticlimacticEventHandler {
	...
	private static final String NBT_KEY_CURSEDGIFT = "anticlimacticlagacy.cursedgift";
	private static final String NBT_KEY_ENABLESPELLSTONE = "anticlimacticlagacy.spellstones_enabled";
	...
	public void onApplyPotion(PotionEvent.PotionApplicableEvent event) {
		if (event.getEntityLiving() instanceof Player) {
			...
			} else if ((SuperpositionHandler.hasCurio(player, anticlimacticItem)
	...
	public void onItemUse(LivingEntityUseItemEvent.Stop event) {
		if (event.getItem().getItem() instanceof CrossbowItem && event.getEntityLiving() instanceof Player) {
			...
			if (!AnticlimacticEnchantmentHelper.hasCustomCrossbowEnchantments(crossbowStack))
				return;
	...
	public void onPlayerClick(PlayerInteractEvent event) {
		if (event instanceof PlayerInteractEvent.RightClickItem || event instanceof PlayerInteractEvent.RightClickBlock || event instanceof PlayerInteractEvent.EntityInteract) {
			if (event.getItemStack().getItem() instanceof CrossbowItem) {
				...
				if (!AnticlimacticEnchantmentHelper.hasCustomCrossbowEnchantments(itemstack))
					return;
				...
				} else if (!event.getPlayer().getProjectile(itemstack).isEmpty() || (CeaselessEnchantment.allowNoArrow.getValue() && AnticlimacticEnchantmentHelper.hasCeaselessEnchantment(itemstack))) {
					if (!CrossbowItem.isCharged(itemstack)) {
	...
	public void onOverlayRender(RenderGameOverlayEvent.PreLayer event) {
		...
		if (event.getOverlay() == ForgeIngameGui.EXPERIENCE_BAR_ELEMENT) {
			...
			if (true) {
				String title = I18n.get("gui.anticlimacticlagacy.blazing_core_bar_title");
				...
				try {
					stringXCorrection = Integer.parseInt(I18n.get("gui.anticlimacticlagacy.blazing_core_bar_offsetX"));
					stringYCorrection = Integer.parseInt(I18n.get("gui.anticlimacticlagacy.blazing_core_bar_offsetY"));
				} catch (Exception ex) {
					// NO-OP
				}
	...
	public void onInventoryGuiInit(ScreenEvent.InitScreenEvent.Post evt) {
		...
		if (screen instanceof EnchantmentScreen || (QuarkHelper.getMatrixEnchanterClass() != null
			...
			AnticlimacticLagacy.packetInstance.send(PacketDistributor.SERVER.noArg(), new PacketEnchantingGUI());
		}
	...
	public void onEntityTick(TickEvent.ClientTickEvent event) {
		if (event.phase == event.phase.END) {
			try {
				...
				AnticlimacticEventHandler.deferredToast.tick(player);

				if (AnticlimacticEventHandler.deferredToast.getCooldown(player) == 1) {
					Minecraft.getInstance().getToasts().addToast(AnticlimacticEventHandler.scheduledToasts.get(0));
					AnticlimacticEventHandler.scheduledToasts.remove(0);

					if (AnticlimacticEventHandler.scheduledToasts.size() > 0) {
	...
	public void miningStuff(PlayerEvent.BreakSpeed event) {
		...
		if (AnticlimacticLagacy.anticlimacticAmulet.hasColor(event.getPlayer(), AmuletColor.GREEN)) {
			miningBoost += 0.25F;
		}
	...
	public void onPlayerTick(LivingEvent.LivingUpdateEvent event) {
		...
		if (event.getEntityLiving() instanceof Player player) {
			...
			if (player instanceof ServerPlayer) {
				...
				AnticlimacticLagacy.anticlimacticItem.handleAnticlimacticFlight(player);
					...
					for (int i = 0; i < list.size(); ++i) {
						...
						if (stack != null) {
							if (stack.is(unwitnessedAmulet)) {
								stack = new ItemStack(anticlimacticAmulet);
								anticlimacticAmulet.setInscription(stack, player.getGameProfile().getName());

								if (AnticlimacticAmulet.seededColorGen.getValue()) {
									anticlimacticAmulet.setSeededColor(stack);
								} else {
									anticlimacticAmulet.setRandomColor(stack);
								}
	...
	public void onConfirmedDeath(LivingDeathEvent event) {
		if (event.getEntityLiving() instanceof ServerPlayer) {
			...
			/*
			 * Updates Anticlimactic Amulet/Scroll of Postmortal Recall possession status for LivingDropsEvent.
			 */
			...
			if (SuperpositionHandler.hasCurio(player, anticlimacticAmulet) || SuperpositionHandler.hasItem(player, anticlimacticAmulet)) {
				postmortalPossession.put(player, anticlimacticAmulet);
	...
	public void onLivingDeath(LivingDeathEvent event) {
		...
		if (event.getEntityLiving() instanceof Player && !event.getEntityLiving().level.isClientSide) {
			...
			if (isPoisonHurt && event.getSource() == DamageSource.MAGIC) {
				...
				event.setCanceled(true);
				player.setHealth(1);
			} else if (SuperpositionHandler.hasCurio(player, AnticlimacticLagacy.anticlimacticItem) || player.getInventory().contains(new ItemStack(AnticlimacticLagacy.anticlimacticItem)) || event.getSource() instanceof DamageSourceNemesisCurse) {
				event.setCanceled(true);
				player.setHealth(1);
		...
		if (event.getRayTraceResult() instanceof EntityHitResult) {
			...
			if (result.getEntity() instanceof Player) {
				...
				if (!player.level.isClientSide) {
					...
					if (AnticlimacticLagacy.anticlimacticAmulet.hasColor(player, AmuletColor.VIOLET)) {
						trigger = true;
						chance += 0.15;
					}
					...
					if (event.getEntity().getTags().contains("anticlimacticlagacy.redirected")) {
						event.setCanceled(true);
						event.getEntity().removeTag("anticlimacticlagacy.redirected");
					}
				}*/
			}
		}
	...
	public void onEntityAttacked(LivingAttackEvent event) {
		...
		if (event.getEntityLiving() instanceof Player player) {
			...
			if (event.getSource().msgId == DamageSource.FALL.msgId) {
				if (AnticlimacticLagacy.anticlimacticAmulet.hasColor(player, AmuletColor.MAGENTA) && event.getAmount() <= 2.0f) {
					event.setCanceled(true);
				}
			}
			...
			if (event.getEntityLiving() instanceof Player victim && SuperpositionHandler.hasFoolsFavor(player)) {
	...
	public void onEntityDamaged(LivingDamageEvent event) {
		...
		if (event.getSource().getDirectEntity() instanceof Player && !event.getSource().getDirectEntity().level.isClientSide) {
			...
			if (AnticlimacticLagacy.anticlimacticAmulet.hasColor(player, AmuletColor.BLACK)) {
	...
	public void onEntityHurt(LivingHurtEvent event) {
		...
		if (event.getSource().getDirectEntity() instanceof AbstractArrow) {
			...
			if (event.getEntityLiving() instanceof Player victimPlayer && SuperpositionHandler.hasFoolsFavor(player)) {
			...
			if (player.getMainHandItem() != null) {
				...
				if (AnticlimacticEnchantmentHelper.hasNemesisCurseEnchantment(mainhandStack)) {
		...
		if (event.getEntityLiving() instanceof Player) {
			...
			if (event.getSource().msgId == DamageSource.FALL.msgId) {
				if (AnticlimacticLagacy.anticlimacticAmulet.hasColor(player, AmuletColor.MAGENTA)) {
	...
	public void playerClone(PlayerEvent.Clone event) {
			...
			for (int i = 0; i < 3; i++) {
				...
				if (inventoryList != null) {
					inventoryList.forEach(stack -> {
						if (!stack.isEmpty() && !anticlimacticlagacy.cursedRing.isItemDeathPersistent(stack) && stack != player.getHeldItemMainhand()) {
		 ...
		 *
			CuriosApi.getCuriosHelper().getEquippedCurios(player).ifPresent(handler -> {
				for (int index = 0; index < handler.getSlots(); index++) {
					if (handler.getStackInSlot(index) != null && !handler.getStackInSlot(index).isEmpty() && !anticlimacticlagacy.cursedRing.isItemDeathPersistent(handler.getStackInSlot(index))) {
		 ...
		 *
			...
			for (int i = 0; i < itemsToDestroy; i++) {
				int randomIndex = AnticlimacticEventHandler.theySeeMeRollin.nextInt(itemsToDestroy);

				while (rolledIndexes.contains(randomIndex)) {
					AnticlimacticEventHandler.theySeeMeRollin.nextInt(itemsToDestroy);
				}
	...
	public void onExperienceDrop(LivingExperienceDropEvent event) {
		...
		if (event.getEntityLiving() instanceof ServerPlayer) {
			if (this.hadAnticlimacticAmulet((Player) event.getEntityLiving()) && !event.getEntityLiving().level.getGameRules().getBoolean(GameRules.RULE_KEEPINVENTORY)) {
				event.setCanceled(true);
			}
	...
	public void onPlayerJoin(PlayerEvent.PlayerLoggedInEvent event) {
		...
		try {
			...
			if (!anticlimacticlagacy.isCSGPresent()) {
		...
		if (!player.level.isClientSide) {
			...
			if (!event.isEndConquered()) {
				if (!player.level.getGameRules().getBoolean(GameRules.RULE_KEEPINVENTORY)) {
					...
					if (SuperpositionHandler.hasPersistentTag(player, AnticlimacticEventHandler.NBT_KEY_ENABLESCROLL)) {
						SuperpositionHandler.unlockSpecialSlot("scroll", event.getPlayer());
					}
					if (SuperpositionHandler.hasPersistentTag(player, AnticlimacticEventHandler.NBT_KEY_ENABLESPELLSTONE)) {
						SuperpositionHandler.unlockSpecialSlot("spellstone", event.getPlayer());
					}
				}
			}
	...
	public void onLogin(LoggedInEvent event) {
		//anticlimacticlagacy.performCleanup();
	}
	...
	public void onItemCrafted(PlayerEvent.ItemCraftedEvent event) {
		if (event.getPlayer() != null && !event.getPlayer().level.isClientSide) {
			if ((event.getInventory().countItem(AnticlimacticLagacy.enchantmentTransposer) == 1 || event.getInventory().countItem(AnticlimacticLagacy.curseTransposer) == 1) && event.getCrafting().getItem() == Items.ENCHANTED_BOOK) {
	...
	public static void grantStarterGear(ServerPlayer player) {
		...
		/*
		 * Handler for bestowing Anticlimactic Amulet to the player, when they first join
		 * the world.
		 */
		...
		if (OmniconfigHandler.isItemEnabled(AnticlimacticLagacy.anticlimacticAmulet))
			if (!SuperpositionHandler.hasPersistentTag(player, AnticlimacticEventHandler.NBT_KEY_ENIGMATICGIFT)) {
				...
				if (!AnticlimacticAmulet.seededColorGen.getValue()) {
					anticlimacticAmulet.setPseudoRandomColor(amuletStack);
				} else {
					anticlimacticAmulet.setSeededColor(amuletStack);
				}
				...
				SuperpositionHandler.setPersistentBoolean(player, AnticlimacticEventHandler.NBT_KEY_ENIGMATICGIFT, true);
			}
		...
		if (OmniconfigHandler.isItemEnabled(AnticlimacticLagacy.cursedRing))
			if (!SuperpositionHandler.hasPersistentTag(player, AnticlimacticEventHandler.NBT_KEY_CURSEDGIFT)) {
				...
				SuperpositionHandler.setPersistentBoolean(player, AnticlimacticEventHandler.NBT_KEY_CURSEDGIFT, true);
			}
	}
	...
	private boolean hadAnticlimacticAmulet(Player player) {
		return postmortalPossession.containsEntry(player, anticlimacticAmulet) || postmortalPossession.containsEntry(player, ascensionAmulet);
	}

	private boolean hadEscapeScroll(Player player) {
		return postmortalPossession.containsKey(player) ? postmortalPossession.containsEntry(player, AnticlimacticLagacy.escapeScroll) : false;
	}

	private boolean hadUnholyStone(Player player) {
		return postmortalPossession.containsKey(player) ? postmortalPossession.containsEntry(player, AnticlimacticLagacy.cursedStone) : false;
	}

	private boolean hadCursedRing(Player player) {
		return postmortalPossession.containsKey(player) ? postmortalPossession.containsEntry(player, AnticlimacticLagacy.cursedRing) : false;
	}
