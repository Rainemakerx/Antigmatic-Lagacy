...
public class AnticlimacticUpdateHandler {
	...
	public static void onConfig(OmniconfigWrapper builder) {
		...
		if (builder.config.getSidedType() != Configuration.SidedConfigType.CLIENT) {
			...
			notificationsEnabled = builder
					.comment("Whether or not Anticlimactic Legacy should show notification in chat when new mod update is available.")
	...
	private static void getNewestVersion() {
		try
		{
			URL url = new URL("https://raw.githubusercontent.com/Extegral/Anticlimactic-Legacy/1.18.X/version.txt");
