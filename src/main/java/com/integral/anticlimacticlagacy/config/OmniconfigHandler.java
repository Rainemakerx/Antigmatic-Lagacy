package com.integral.anticlimacticlagacy.config;
...
public class OmniconfigHandler {
	...
	public static void initialize() {
		OmniconfigWrapper configBuilder = OmniconfigWrapper.setupBuilder("anticlimacticlagacy-common", true, "2.1");
		...
		OmniconfigWrapper clientBuilder = OmniconfigWrapper.setupBuilder("anticlimacticlagacy-client", true, "2.1");
	...
	private static void loadCommon(final OmniconfigWrapper builder) {
		...
		builder.pushCategory("Accessibility Options",
				...
				"Want to disable Anticlimactic Amulet/Ring of the Seven Curses from being granted to player when they spawn?" + System.lineSeparator() +
				"HERE IS THE PLACE AND TIME, COME ON AND SLAM!" + System.lineSeparator() + System.lineSeparator() +
				"Please note that as of release 2.6.0 of Anticlimactic Lagacy, those options are automatically generated" + System.lineSeparator() +
				...
				"pointing out to such option here: https://github.com/Extegral/Anticlimactic-Lagacy/issues");
		...
		customDungeonLootEnabled = builder
				.comment("Whether or not this mod should add any custom loot to dungeon chests' loot tables. This options is mainly "
						...
						+ "modifying what Anticlimactic Lagacy adds to loot tables currently exist.")
				.sync()
		...
		crashOnUnnamedPool = builder
				.comment("Whether or not Anticlimactic Lagacy should purposefully crash client whenever any mod tries to inject unnamed "
		...
		retriggerRecipeUnlocks = builder
				.comment("If true, Anticlimactic Lagacy will cycle through each player's recipe book and trigger 'minecraft:recipe_unlocked' criterion trigger for everything that they have unlocked upon player joining the world.")
