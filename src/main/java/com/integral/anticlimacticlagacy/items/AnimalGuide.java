...
public class AnimalGuide extends ItemBase implements Vanishable {
	...
	public void appendHoverText(ItemStack stack, Level worldIn, List<Component> list, TooltipFlag flagIn) {
		if (Screen.hasShiftDown()) {
			ItemLoreHelper.addLocalizedString(list, "tooltip.anticlimacticlagacy.animalGuide1");
			ItemLoreHelper.addLocalizedString(list, "tooltip.anticlimacticlagacy.animalGuide2");
			ItemLoreHelper.addLocalizedString(list, "tooltip.anticlimacticlagacy.animalGuide3");
			...
			if (Minecraft.getInstance().player != null && SuperpositionHandler.isTheCursedOne(Minecraft.getInstance().player)) {
				...
				ItemLoreHelper.addLocalizedString(list, "tooltip.anticlimacticlagacy.animalGuide4");
				ItemLoreHelper.addLocalizedString(list, "tooltip.anticlimacticlagacy.animalGuide5");
			}
		} else {
		...
		if (Minecraft.getInstance().player != null && Minecraft.getInstance().player.isCreative()) {
			ItemLoreHelper.addLocalizedString(list, "tooltip.anticlimacticlagacy.void");
			ItemLoreHelper.addLocalizedString(list, "tooltip.anticlimacticlagacy.animalGuideCreative1");
			ItemLoreHelper.addLocalizedString(list, "tooltip.anticlimacticlagacy.animalGuideCreative2");
		}
	}
	...
	public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand hand) {
		if (player.isCreative()) {
			if (!player.getLevel().isClientSide) {
				...
				AnticlimacticLagacy.logger.info("Checking up the list of " + entries.size() + " entities to find which are affected by curse-altering effect of Guide to Animal Companionship...");
				ForgeRegistries.ENTITIES.getEntries().forEach(entry -> {
					...
					if (TamableAnimal.class.isAssignableFrom(type.getBaseClass()) || animalExclusionList.contains(type.getRegistryName())) {
						AnticlimacticLagacy.logger.info(" - Entity: " + type.getRegistryName() + ", Class: " + type.getBaseClass());
					}
				});

				AnticlimacticLagacy.logger.info("The analysis is complete.");
				player.sendMessage(new TranslatableComponent("message.anticlimacticlagacy.animal_analysis_complete").withStyle(ChatFormatting.DARK_PURPLE), player.getUUID());
			}
