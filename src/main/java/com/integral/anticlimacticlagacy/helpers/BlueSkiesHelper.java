...
public class BlueSkiesHelper {
	...
	static {
		...
		try {
			player = Class.forName("com.lagacy.blue_skies.capability.SkiesPlayer");
			iface = Class.forName("com.lagacy.blue_skies.capability.util.ISkiesPlayer");
		...
		AnticlimacticLagacy.logger.info("Blue Skies detected: " + MOD_PRESENT);
	}
			...
			if (capability != null) {
				...
				if (maybeHealth instanceof Float) {
					...
					if (health.isNaN()) {
						...
						player.setHealth(0F);
						AnticlimacticLagacy.logger.info("Fixed NaN natural health for player: " + player.getGameProfile().getName());
					}
				}
			}
		} catch (Exception ex) {
